...
if($task == "dologin") {
  ...
  if(trim($input_user) == "" || trim($input_pass) == "") {
    ...
    if(crypt($input_user, $admin_user) == crypt($admin_user, $admin_user) && crypt($input_pass, $admin_pass) == crypt($admin_pass,$admin_pass)) {
      ...
      exit;
    } elseif(crypt($input_user, $dash_user) == crypt($dash_user, $dash_user) && crypt($input_pass, $dash_pass) == crypt($dash_pass,$dash_pass)) {
      setcookie("representmap_user", crypt($input_user, $dash_user), time()+3600000);
      setcookie("representmap_pass", crypt($input_pass, $dash_pass), time()+3600000);
